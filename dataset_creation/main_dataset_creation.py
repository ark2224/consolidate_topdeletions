...
def main():
    ...
    Sams_dict = {}
    csvfile = Path('data/batch_2_sams.csv')
    ...
    for k,v in Sams_dict.items():
        if k in full_seq_id_2_frag_pass:#will only be ~3300
            ...
            for sam_name in v:
                ...
                # os.system('samtools fastq /content/tmpfile.sam > /content/tmp_file.fastq')
                # with open('tmp_file.fasta', 'w') as fasta:
                #     fasta.write(f'>{k}\n')
                #     fasta.write(TAF + seq + TAR)
                # os.system(f'minimap2 -ax map-ont -A 2 -B 2 -O 2,40 -E 2,0 --end-bonus 50 /content/tmp_file.fasta /content/tmp_file.fastq > /content/{k}_{gene_count[gene_id]}.sam')
